...
public class StringUtil {
    ...
    public static String hiddenMobile(String mobile){
        String ret = "";
        if(mobile != null && !"null".equalsIgnoreCase(mobile) && verifyMobileFormat(mobile)){
            ret = hidden(mobile.length() - 3) + mobile.substring(mobile.length() - 3);
        }
        return ret;
    }
    ...
    public static boolean verifyMobileFormat(String mobile){
        if (isBlank(mobile))
            return false;
        else if (mobile.length() !=11)
            return false;
        else if (!Pattern.matches("^13\\d{9}$", mobile) &&
                !Pattern.matches("^15\\d{9}$", mobile) && !Pattern.matches("^18\\d{9}$", mobile) && !Pattern.matches("^14\\d{9}$", mobile) && !Pattern.matches("^17\\d{9}$", mobile))
            return false;
        else
            return true;
    }
